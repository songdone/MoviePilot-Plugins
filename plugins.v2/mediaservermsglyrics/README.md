...
- 电视端歌曲标题字号由 24 提高到 30，歌手/专辑由 17 提高到 21，音质信息由 15 提高到 18，远距离观看更清晰。
- 左侧歌曲信息组整体下移，音质行与右侧进度条组成统一的底部视觉带，不再紧贴封面玻璃托盘。
- 直接移除玻璃托盘下方的大块黑色阴影；封面本体只保留低透明度柔和阴影，并提高玻璃边缘亮度，质感更轻透。
- 放大后的标题、歌手、专辑和音质信息按歌曲缓存成独立图层，长标题只在切歌时排版一次，不占用后续 30 FPS 歌词帧预算。

## 1.1.8 更新

- 恢复 Apple TV 端的封面取色高斯模糊动态渐变，不再只显示一张固定模糊背景；换歌后会跟随新封面重新取色。
- 色团位移幅度比早期版本提高约 50%，循环时间缩短到 24 秒，让微动流动感更容易看见，同时保持克制、避免抢夺歌词注意力。
- 手机与 PC 歌词页同步增强 GPU 色场位移并缩短动画周期；系统启用“减少动态效果”时仍会自动停用微动。
- 动态色场改由独立线程以 3 FPS 低频合成，歌词、进度条和 H.264 视频仍独立保持 30 FPS；背景生成偶发变慢不会卡住歌词帧。
- 新增“电视歌词同步补偿”，默认让电视歌词时间轴提前 `0.9` 秒，抵消编码及 UnPlay 缓冲造成的轻微延迟，可在 `-2～3` 秒内按实际设备微调。
- Plex 播放状态每 0.5 秒在后台刷新；网络查询不进入视频逐帧线程。

## 1.1.7 更新

- 投屏改为低延迟优先：检测到 Intel Quick Sync 时保持 `3840×2160 / 30 FPS`；没有核显编码时自动使用 `1920×1080 / 30 FPS`，不再让 N305 用 libx264 强行放大并编码 4K。
- 电视逐帧渲染不再同步等待 Plex `sessions()` 网络请求，进度刷新改到后台执行，避免每隔约一秒卡住视频帧。
- 封面背景先在小尺寸完成高斯模糊再放大，阴影只处理局部图层，降低切歌时的首帧生成时间。
- 1.1.7 曾暂时停止电视背景微动以保护帧预算；1.1.8 已改成独立后台渲染后恢复动态色场。
- UnPlay 重连时保留已合成的场景缓存；调整编码器关闭顺序，避免 `I/O operation on closed file` 竞态和反复重建画面。
- GOP 缩短为一秒并让 MPEG-TS 主动刷新数据包、重发头信息，改善起播和断线重连速度。
- 新增“电视流局域网直连地址”：Apple TV 可直接从 NAS 取流，绕过公网域名回流及反向代理缓冲。
...
5. 为获得最低延迟，建议在“电视流局域网直连地址”填写 Apple TV 能访问的 MoviePilot 地址，例如当前 Compose 映射为 `3032:3000` 时填写 `http://NAS局域网IP:3032`。手机通知仍使用公网歌词地址，两者互不影响。
6. 若电视歌词仍稍晚于声音，提高“电视歌词同步补偿”；若歌词提前，则减小该值。默认值为 `0.9` 秒，修改后保存并重新投屏生效。

SSDP/DLNA 只负责局域网设备发现和控制；实际视频仍通过 HTTP 传输。Docker bridge 可能影响 UDP 组播发现，但不会天然让 HTTP 视频更卡。找不到设备时使用 UnPlay 备用 IP，只是跳过自动发现，不会降低视频质量或帧率。
...
电视流会根据编码器自动选择清晰度：Intel Quick Sync 使用 `4K/30`；libx264 软件编码使用低延迟 `1080p/30`。1080p 会由 Apple TV/电视进行硬件缩放，清晰度略低于原生 4K，但可显著降低首帧等待、切歌延迟和断流风险。
