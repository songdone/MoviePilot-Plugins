...
from .lyrics_runtime import LyricsRuntime


class MediaServerMsgLyrics(_PluginBase):
    """
    媒体服务器通知插件
    ...
    # 插件基本信息
    plugin_name = "媒体库服务器通知·实时歌词"
    # 插件描述
    plugin_desc = "发送媒体服务器通知；Plex 音乐提供实时歌词页与 UnPlay 电视投屏。"
    # 插件图标
    ...
    # 作者主页
    author_url = "https://github.com/songdone/MoviePilot-Plugins"
    # 插件配置项ID前缀
    plugin_config_prefix = "mediaservermsglyrics_"
    # 加载顺序
    plugin_order = 14
    # 可使用的用户级别
    auth_level = 1
    ...
    def __init__(self):
        ...
        self._lyrics_runtime = None
        self._initialize_lifecycle_state()
        logger.debug("媒体服务器实时歌词通知插件初始化完成")
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._lyrics_runtime:
            self._lyrics_runtime.close()
        ...
        with self._lifecycle_condition:
            if config:
                ...
                self._lyrics_enabled = config.get("lyrics_enabled", True)
                self._lyrics_online_fallback = config.get("lyrics_online_fallback", True)
            ...
            self._aggregate_timers = {}
            self._owned_timers = set()
            self._lyrics_runtime = LyricsRuntime(self, logger)
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        """注册微信可匿名打开的歌词页、播放状态和同源封面代理。"""
        return [
            {
                "path": "/lyrics",
                "endpoint": self.lyrics_page,
                "methods": ["GET"],
            ...
            {
                "path": "/lyrics/cast/devices",
                "endpoint": self.lyrics_cast_devices,
                "methods": ["GET"],
                "allow_anonymous": True,
                "response_class": JSONResponse,
                "include_in_schema": False,
            ...
            {
                "path": "/lyrics/tv.ts",
                "endpoint": self.lyrics_tv_mpegts,
                "methods": ["GET"],
                "allow_anonymous": True,
            ...
            {
                "path": "/lyrics/tv.mjpg",
                "endpoint": self.lyrics_tv_stream,
                "methods": ["GET"],
                "allow_anonymous": True,
                "response_class": StreamingResponse,
                "include_in_schema": False,
    ...
    def lyrics_page(self, sid: Optional[str] = None) -> HTMLResponse:
        """返回移动端实时歌词页面；sid 的有效性由状态接口持续校验。"""
        page_path = Path(__file__).with_name("lyrics.html")
        return HTMLResponse(
            page_path.read_text(encoding="utf-8"),
            headers={"Cache-Control": "no-store"},
        )

    def lyrics_state(self, sid: str) -> JSONResponse:
        """返回指定临时歌词会话的播放进度和歌词时间轴。"""
        runtime = self._lyrics_runtime
        data = runtime.get_state(sid) if runtime else {
            "ok": False,
            "message": "实时歌词服务尚未启动。",
        }
        return JSONResponse(
    ...
    def lyrics_cover(self, sid: str) -> Response:
        """代理 Plex 封面，避免在通知链接和浏览器中泄露 Plex Token。"""
        content, media_type = self._lyrics_runtime.get_cover(sid) if self._lyrics_runtime else (None, "image/jpeg")
        if not content:
    ...
    def lyrics_cast_devices(self, force: bool = False) -> JSONResponse:
        """自动发现同一局域网中的 UnPlay，并返回网页设备选择器所需信息。"""
        runtime = self._lyrics_runtime
        data = runtime.discover_unplay_devices(force=force) if runtime else {
            "ok": False,
            "devices": [],
            "message": "实时歌词服务尚未启动。",
        }
        data["devices"] = [
            {key: device.get(key) for key in ("id", "name", "detail", "source")}
            for device in data.get("devices", [])
            if isinstance(device, dict)
        ]
        return JSONResponse(
            content=data,
            status_code=200 if data.get("ok") else 503,
            headers={"Cache-Control": "no-store"},
        )

    def lyrics_cast(self, sid: str, device_id: Optional[str] = None) -> JSONResponse:
        """让 MoviePilot 从服务端调用局域网 UnPlay，避免浏览器跨域与混合内容限制。"""
        runtime = self._lyrics_runtime
        data = runtime.start_unplay_cast(sid, device_id or "") if runtime else {
            "ok": False,
            "message": "实时歌词服务尚未启动。",
        }
        ...
        return JSONResponse(
            content=data,
            status_code=200 if data.get("ok") else 400,
            headers={"Cache-Control": "no-store"},
        )

    def lyrics_tv_stream(self, channel: str):
        """返回兼容旧版的无声 MJPEG 调试流。"""
        runtime = self._lyrics_runtime
        if not runtime or not runtime.tv_channel_exists(channel):
            return Response(status_code=404, headers={"Cache-Control": "no-store"})
        return StreamingResponse(
            runtime.iter_tv_stream(channel),
    ...
    def lyrics_tv_mpegts(self, channel: str, v: str = ""):
        """返回低延迟 H.264/MPEG-TS；v 用于隔离每次 UnPlay 投屏任务。"""
        runtime = self._lyrics_runtime
        if not v or not runtime or not runtime.tv_channel_exists(channel, v):
            return Response(status_code=404, headers={"Cache-Control": "no-store"})
        return StreamingResponse(
            runtime.iter_tv_mpegts(channel, v),
                                ...
                                'content': [
                                    {
                                        'component': 'VSwitch',
                                        'props': {
                                            'model': 'lyrics_enabled',
                                            'label': 'Plex音乐通知打开实时歌词',
                                        }
                                    }
                                ]
                            },
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'model': 'lyrics_public_url',
                                            'label': '歌词页公网地址',
                                            'placeholder': 'https://mp.playsong.cn',
                                            'hint': '填写MoviePilot反向代理域名，不要以斜杠结尾',
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'label': '音乐路径映射（可选）',
                                            'rows': 2,
                                            'placeholder': '/volume1/music => /media/music',
                                            'hint': 'Plex与MoviePilot看到的音乐路径不同时填写，每行一条',
                                            'persistent-hint': True,
                                        }
                                    }
                                ]
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'model': 'unplay_host',
                                            'label': 'UnPlay 备用 IP（通常不用填）',
                                            'placeholder': '192.168.1.111:9030',
                                            'hint': '默认会自动发现同一局域网设备；仅当 Docker 网络阻止 SSDP 组播时，才在这里填写 UnPlay → 设置 → HTTP 投屏显示的地址',
                                            'persistent-hint': True,
                                        }
                                    }
                                ]
    ...
    def _handle_webhook_event(self, event: Event):
        ...
        try:
            ...
            # 歌词会话需要接收 play/pause/resume/stop 全部事件；其中暂停和恢复
            # 不属于普通通知类型，因此必须在后续通知过滤之前处理。
            lyrics_sid = (
                self._lyrics_runtime.handle_event(event_info)
                if self._lyrics_runtime else None
            )
            ...
            plex_track = self._get_plex_track_metadata(event_info)
            if plex_track and plex_track.get("album"):
                message_texts.append(f"专辑：{plex_track['album']}")
            ...
            # 处理播放链接
            play_link = None
            if lyrics_sid and self._lyrics_enabled:
                play_link = self._build_lyrics_link(lyrics_sid)
            elif self._add_play_link:
    ...
    @classmethod
    def _get_plex_track_metadata(cls, event_info: WebhookEventInfo) -> Optional[Dict[str, str]]:
        """从原始 Plex Webhook 提取歌词页和通知正文需要的音乐字段。"""
        if cls._clean_metadata_text(getattr(event_info, 'channel', None)).lower() != "plex":
            return None
        payload = getattr(event_info, 'json_object', None)
        metadata = payload.get("Metadata") if isinstance(payload, dict) else None
        if not isinstance(metadata, dict) or cls._clean_metadata_text(metadata.get("type")).lower() != "track":
            return None
        return {
            "title": cls._clean_metadata_text(metadata.get("title")),
            "artist": cls._clean_metadata_text(metadata.get("grandparentTitle") or metadata.get("originalTitle")),
            "album": cls._clean_metadata_text(metadata.get("parentTitle")),
        }

    def _build_lyrics_link(self, sid: str) -> str:
        """构造企业微信通知点击后打开的公网歌词页链接。"""
        base_url = (self._lyrics_public_url or "https://mp.playsong.cn").rstrip("/")
        return f"{base_url}/api/v1/plugin/MediaServerMsgLyrics/lyrics?sid={sid}"
    ...
    @cached(
        region="MediaServerMsgLyrics",     # 缓存区域，用于隔离不同插件的缓存
